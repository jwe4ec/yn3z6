...
impList_contemp <- as.mitml.list(split(imps2$contemp_imps$red_cnDoSS_contemp_imps, 
                                       imps2$contemp_imps$red_cnDoSS_contemp_imps$imp_num))
impList_lagged <- as.mitml.list(split(imps2$lagged_imps$red_cnDoSS_lagged_imps,
                                      imps2$lagged_imps$red_cnDoSS_lagged_imps$imp_num))
...
contemp_meanDSS_wth <- create_results_list(modelList, pooled, 2)
...
modelList <-
  with(impList_contemp,
       lme(fixed = drtotl_m_imp_pomp ~ time0 + meanDSS_wth_pomp + meanDSS_btw_cent_pomp,
           ...
           method = "REML"))
pooled <- testEstimates(modelList, extra.pars = TRUE, df.com = df_tot_eff_model)
contemp_meanDSS_wth_pomp <- create_results_list(modelList, pooled, 2)
...
modelList <-
  with(impList_lagged,
       lme(fixed = drtotl_m_imp ~ time0 + lmeanDSS_wth + lmeanDSS_btw_cent,
           ...
           method = "REML"))
pooled <- testEstimates(modelList, extra.pars = TRUE, df.com = df_tot_eff_model)
lagged_meanDSS_wth <- create_results_list(modelList, pooled, 2)
...
modelList <-
  with(impList_lagged,
       lme(fixed = drtotl_m_imp_pomp ~ time0 + lmeanDSS_wth_pomp + lmeanDSS_btw_cent_pomp,
           ...
           method = "REML"))
pooled <- testEstimates(modelList, extra.pars = TRUE, df.com = df_tot_eff_model)
lagged_meanDSS_wth_pomp <- create_results_list(modelList, pooled, 2)
...
# Error: optim problem, convergence error code = 1. Resolved with msMaxIter and
...
result_itt_red_cnDoSS <- list("contemp_meanDSS_wth" = contemp_meanDSS_wth, 
...
save(result_itt_red_cnDoSS, file = "./results/result_itt_red_cnDoSS.RData")
